    ...
    for (j in 1:p){
      ...
      if (j == 1){
        ...
        scoreXY[(i - 1) * p + j, 2] <- eta0.eta1.eta2.gamma.x[df$id, 1] + 
          df$time1 * eta0.eta1.eta2.gamma.x[df$id, 2] +
          df$time2 * eta0.eta1.eta2.gamma.x[df$id, 3] + 
          kappa * long_dat$CHG[long_dat$id == i][j] 
      }
...
getBLSGM_TVCchg <- function(dat, p, init, manifests, rateX, extratry = extratry, loop = loop){
  ...
  model_mx <- mxModel(name = "BLSGM with TVC change in each interval", type = "RAM", 
                      ...
                      mxPath(from = paste0("lx", 1:(p - 1)), to = paste0("lx", 2:p), arrows = 1, free = F, values = 1),
                      ############################################################################################
...
###########################
tmpTVCchg <- try(getBLSGM_TVCchg(dat = dat[[2]], p = p, init = init, manifests = c(paste0("TVC", 1:p), paste0("Y", 1:p), "TIC"), 
                                 rateX = X_rate, extratry = extratry, loop = loop))
out <- getEstimation(model = tmpTVCchg, paraNames = paraNames, p = p)
