    ...
    for (j in 1:p){
      ...
      if (j == 1){
        ...
        scoreXY[(i - 1) * p + j, 2] <- eta0.eta1.eta2.gamma.x[df$id, 1] + 
          df$time1 * eta0.eta1.eta2.gamma.x[df$id, 2] +
          df$time2 * eta0.eta1.eta2.gamma.x[df$id, 3] + 
          kappa * long_dat$CHG_BL[long_dat$id == i][j] 
      }
...
getBLSGM_TVCchgBL <- function(dat, p, init, manifests, rateX, extratry = extratry, loop = loop){
  ### Define latent variables
  latents <- c("eta0Ys", "eta1Ys", "eta2Ys", "eta0x", "eta1x", paste0("lx", 1:p), paste0("dx", 2:p), paste0("deltax", 2:p), paste0("Deltax", 2:p))
  ...
  model_mx <- mxModel(name = "BLSGM with TVC change from baseline", type = "RAM", 
                      ...
                      mxPath(from = paste0("deltax", 2:p), to = paste0("Deltax", 2:p), arrows = 1, free = F, values = 1),
                      mxPath(from = paste0("Deltax", 2:p), to = paste0("lx", 2:p), arrows = 1, free = F, values = 1),
                      #### Define autoregressive paths
                      mxPath(from = paste0("Deltax", 2:(p - 1)), to = paste0("Deltax", 3:p), arrows = 1, free = F, values = 1),
                      mxPath(from = "lx1", to = paste0("lx", 2:p), arrows = 1, free = F, values = 1),
                      ############################################################################################
...
###########################
tmpTVCchgBL <- try(getBLSGM_TVCchgBL(dat = dat[[2]], p = p, init = init, manifests = c(paste0("TVC", 1:p), paste0("Y", 1:p), "TIC"), 
                                     rateX = X_rate, extratry = extratry, loop = loop))

out <- getEstimation(model = tmpTVCchgBL, paraNames = paraNames, p = p)
