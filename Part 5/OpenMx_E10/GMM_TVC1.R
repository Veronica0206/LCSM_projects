    ...
    for (j in 1:p){
      ...
      if (j == 1){
        ...
        scoreXY[(i - 1) * p + j, 2] <- eta0.eta1.eta2.gamma.x[df$id, 1] + 
          df$time1 * eta0.eta1.eta2.gamma.x[df$id, 2] +
          df$time2 * eta0.eta1.eta2.gamma.x[df$id, 3] + 
          KAPPA[df$id] * long_dat$Rate[long_dat$id == i][j] 
      }
...
getBLSGMM_TVCslp <- function(dat, p, c, init, beta, manifests, rateX, extratry = 10, loop = 20){
  ...
  GMM_mx <- mxModel("BLSGMM with TVC interval specific slopes", 
...
###########################
tmpGMMTVCslp <- try(getBLSGMM_TVCslp(dat = dat[[2]], p = p, c = c, init = initial, beta = log_Beta,
                                     manifests = c(paste0("TVC", 1:p), paste0("Y", 1:p), "TIC"), 
                                     rateX = X_Rate, extratry = 10, loop = 20))

outTVC <- getEstimation(model = tmpGMMTVCslp, paraNames = paraNames, p = p, c = c)
